...
private:
    ...
    void generate_diagonal_wavefront_path() {
        ...
        std::cout << "Generated wavefront path with " << path_idx
    ...
    double build_position_tables() {
        ...
        std::cout << "Built constraint tables in " << elapsed << " ms" << std::endl;
        std::cout << "Corner pieces: TL=" << corner_tl_count
                  << ", TR=" << corner_tr_count[1] // Sample count
                  << ", Interior pieces for constraints (1,1): " << interior_count[1][1] << std::endl;
    ...
    bool solve(bool first_only = false) {
        ...
        std::cout << "Using Akos-optimized solver (packed format + wavefront order)" << std::endl;
        ...
        if (solve_time > 0 && placements_tried > 0) {
            ...
            std::cout << "Placements per second: " << static_cast<size_t>(placements_per_second) << std::endl;
            std::cout << "Overall placements per second: " << static_cast<size_t>(overall_placements_per_second) << std::endl;

            double overhead = table_time / total_time * 100.0;
            std::cout << "Overhead: " << std::fixed << std::setprecision(6) << table_time << " ms (" << std::fixed << std::setprecision(6) << overhead << "%)" << std::endl;
        }
